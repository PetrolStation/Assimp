...
public:
    ...
    Animation(const std::string& animationPath, PetrolEngine::Mesh& model)
    {
        Assimp::Importer importer;
        const aiScene* scene = importer.ReadFile(animationPath, aiProcess_Triangulate);
        assert(scene && scene->mRootNode);
        auto animation = scene->mAnimations[0];
        std::cout<<"Here is a value of "<<model.m_BoneCounter<<std::endl;
    ...
    inline const std::map<std::string,PetrolEngine::BoneInfo>& GetBoneIDMap() 
    { 
        return m_BoneInfoMap;
    }

private:
    void ReadMissingBones(const aiAnimation* animation, PetrolEngine::Mesh& model)
    {
    ...
    std::map<std::string, PetrolEngine::BoneInfo> m_BoneInfoMap;
};
